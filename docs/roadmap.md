...
| P1-31 | Configure external HDD as ZFS pool `external` (PBS + replication) | ❌ | **Depends on P1-30 Datenmigration (rsync ext. HDD → TrueNAS).** Format HDD as single-disk ZFS pool. Datasets: `external/pbs` (NFS share → PBS VM datastore on PVE), `external/replication` (ZFS replication target from `data` pool). ⚠️ Export pool gracefully before unplugging HDD. |
