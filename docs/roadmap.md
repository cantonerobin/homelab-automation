...
| P1.5-10 | Implement inter-VLAN firewall rules in Unifi Dream Machine | ✅ | Terraform-managed via `paultyng/unifi` provider (`terraform/unifi/`). Legacy LAN_IN rules: established/related, MGMT→any, DNS, Server→DMZ, Client→DMZ, k3s cluster, default deny (192.168.0.0/16). Test playbook: `ansible/playbooks/firewall-test.yml`. |
